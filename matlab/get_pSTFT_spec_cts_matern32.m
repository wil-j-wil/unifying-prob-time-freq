function spec = get_pSTFT_spec_cts_matern32(freqs,lamx,varx,om)
...
%             + 1/2*varx(d)./(1+lamx(d).^2-2*lamx(d)*cos(omegas+om(d))); 
  spec(d,:) = 2 .* varx(d) .* lamx(d).^3 .* (lamx(d).^2 + (omegas-om(d)).^2).^-2 ...
            + 2 .* varx(d) .* lamx(d).^3 .* (lamx(d).^2 + (omegas+om(d)).^2).^-2;
