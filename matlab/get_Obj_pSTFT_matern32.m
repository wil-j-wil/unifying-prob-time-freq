function [Obj,varargout] = get_Obj_pSTFT_matern32(theta,vary,specTar,minVar,limOm,limLam,bet);

% Will Wilkinson 20181026
% calculates spectral density and its gradients for the spectral mixture GP
% with Matern 3/2 kernel function. Based on Richard Turner's original
% code.
...
%   spec = spec + 2 * mVar(d) .* lam(d).^3 .* (alp1_c.^-2 + alp2_c.^-2);
  spec = spec + 2 * cVar(d) .* lam(d).^3 .* (alp1_c.^-2 + alp2_c.^-2);
...
if nargout>1
  ...
  for d=1:D
    ...
    alp1_c = lam(d).^2 + (omegas-om(d)).^2;
    alp2_c = lam(d).^2 + (omegas+om(d)).^2; 
%     dspecdtransVar = (mVar(d)-minVar(d)) .* 2 .* lam(d).^3 .* (alp1_c.^-2 + alp2_c.^-2);
    dspecdtransVar = (mVar(d)-minVar(d)) .* 2 .* (1-lam(d).^2) .* lam(d).^3 .* (alp1_c.^-2 + alp2_c.^-2);
    dObjdtransVar(d) = sum(dObjdspec.*dspecdtransVar); 
    
    
    % Derivative wrt transformed centre frequency
%     dspecdom = 8 .* mVar(d) .* lam(d).^3 * (alp1_c.^-3 .* (omegas-om(d)) - alp2_c.^-3 .* (omegas+om(d)));
    dspecdom = 8 .* mVar(d) .* (1-lam(d).^2) .* lam(d).^3 * (alp1_c.^-3 .* (omegas-om(d)) - alp2_c.^-3 .* (omegas+om(d)));
...
%                           (alp1_c.^-2 + alp2_c.^-2));
%     dspecdlam = 2 .* mVar(d) .* lam(d).^2 .* ( 3 .* (alp1_c.^-2 + alp2_c.^-2) ...
%                                 - 4 .* lam(d).^2 .* (alp1_c.^-3 + alp2_c.^-3));
    dspecdlam = 2 .* mVar(d) .* lam(d).^2 .* ( (3 .* (1 - lam(d).^2) - 2 .* lam(d).^2) .* (alp1_c.^-2 + alp2_c.^-2) ...
                                - 4 .* (1 - lam(d).^2) .* lam(d).^2 .* (alp1_c.^-3 + alp2_c.^-3));
