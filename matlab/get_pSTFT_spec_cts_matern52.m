function spec = get_pSTFT_spec_cts_matern52(freqs,lamx,varx,om)
...
%             + 1/2*varx(d)./(1+lamx(d).^2-2*lamx(d)*cos(omegas+om(d))); 
  spec(d,:) = (8/3) .* varx(d) .* lamx(d).^5 .* (lamx(d).^2 + (omegas-om(d)).^2).^-3 ...
            + (8/3) .* varx(d) .* lamx(d).^5 .* (lamx(d).^2 + (omegas+om(d)).^2).^-3;
